=== vdperanto ===

Contributors: priyanshumittal
Tags: one-column, two-columns, full-width-template, threaded-comments, right-sidebar, custom-menu, sticky-post, translation-ready, featured-images, blog, theme-options, e-commerce
Requires at least: 4.5
Tested up to: 5.9
Stable tag: 2.1.3
Requires PHP: 5.4
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

== Description ==

A Business theme which is ideal for creating a corporate / business website. Vdperanto theme is a child theme of BusiProf WordPress Theme. Those who need the white background-based theme can use this child theme. Vdperanto theme comes with various locales


# Basic Setup of the Theme.
-----------------------------------
Fresh installation!

1. Upload the vdperanto Theme folder to your wp-content/themes folder.
2. Activate the theme from the WP Dashboard.
3. Done!

== Keyboard navigation ==

Keyboard navigation is working properly in the forward direction by using the tab key. For the reverse direction, Shift+Tab key is working on all the parent menus and if the dropdown menus are open then it is working on it. The focus is moving back to the parent menu having submenus from the next menu in a reverse direction.

# Top Navigation Menu:
- Default the page-links start from the left! Use the Menus function in Dashboard/Appearance to rearrange the buttons and build your own Custom-menu. DO NOT USE LONG PAGE NAMES, Maximum 14 letters/numbers incl. spaces!
- Read more here: http://codex.wordpress.org/WordPress_Menu_User_Guide

=============Page Templates======================
1. Contact  Page Tempalte:- Create a page as you do in WordPress and select the page template with the name 'Contact'


===========Front Page Added with the theme=================
1 It has header(logo + menus),Home Featured Image, services,recent comments widgets and footer.

======Site Title and Description=============
Site Title and its description in not shown on home page besides this both are used above each page / post along with the search field.

Support
-------
Do you enjoy this theme? Send your ideas - issues - on the theme formn. Thank you!

== Widgets ==

In WordPress 5.8, the default widget title is showing in the H2 tag if you want you can change it in the H3 tag.

== License ==

Busiprof WordPress Theme, Copyright (C) 2013-2022, priyanshumittal
Busiprof is distributed under the terms of the GNU GPL

vdperanto WordPress Theme is the child theme of Busiprof Theme, Copyright (C) 2017-2022 priyanshumittal
vdperanto WordPress Theme is distributed under the terms of the GNU GPL

== Changelog ==

@Version 2.1.3
* Fixed the site title & description style issue in header overlay layout.

@Version 2.1.2
* Fixed some style issues.

@Version 2.1.1
* Added the recommended plugin for WooCommerce on options page.

@version 2.1
1. Fixed logo style issue in standard header layout.
2. Changed some strings.

@version 2.0.9
1. Fixed widget style issues with WordPress 5.8.

@version 2.0.8
1. Fixed the theme check warnings.

@version 2.0.7
1. Fixed the option page style issue in responsive view.

@version 2.0.6
1. Fixed the logo and site title issue in responsive view and blog section empty markup issue.

@version 2.0.5
1. Fixed the blog meta setting issue in masonry layout.

@version 2.0.4
1. Fixed the admin bar issue with the sticky header.

@version 2.0.3
1. Fixed the standard header layout breadcrumb style issue.
2. Fixed the escaping and prefix issues.

@version 2.0.2
1. Fixed header section style issue in responsive view.
2. Escaped the pingback_url().

@version 2.0.1
1. Added contact form 7 image license.
2. Corrected escaping function.

@version 2.0
1. Implement the Accessibility including skip to content and keyboard navigation.
2. Changed the screenshot image.
3. Added the Header, Service, blog & testimonial section variation.
4. Fixed escaping and sanitizing issues.
5. Fixed all the other issues.

@Version 1.24
1. Fixed data migrate.

@Version 1.23
1. Update String.

@Version 1.22
1. Remove subject tag.

@Version 1.21
1. Updated String

@Version 1.20
1. Tag Revise

@Version 1.2
1. Updated Strings(Suggested by Transifex).

@Version 1.1
1. Update menu color.

@Version 1.09
1. Update Screenshot image.

@Version 1.08
1. Relesed.

== External resources ==

Custom control - Image Radio Button Custom Control:
Copyright: Anthony Hortin
License: GNU General Public License v2 or later
Source: https://github.com/maddisondesigns/customizer-custom-controls

Masonry JS:
Copyright: (C) 2007 Free Software Foundation
License: Under the GNU General Public License v3.0
Source: https://github.com/jmlp-131092/mp-mansory.js

== Images ==

* Image used in screenshot, License CC0 Public Domain
https://pxhere.com/en/photo/1522087

* Images used in Blog radio button, License CC0 Public Domain
https://pxhere.com/en/photo/1456661
https://pxhere.com/en/photo/1325885
https://pxhere.com/en/photo/1567131

* Image used for the Contact Form 7
License: Under the terms of the GNU General Public License
Source: https://wordpress.org/plugins/contact-form-7/

* Image used for the WooCommerce
License: Under the terms of the GNU General Public License
Source: https://wordpress.org/plugins/woocommerce/

* Images on /images folder
Copyright (C) 2017-2022, Webriti and available as [GPLv2](https://www.gnu.org/licenses/gpl-2.0.html)
